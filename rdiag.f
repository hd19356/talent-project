c rdiag  : diagonalize an arbitrary real matrix
c reore  : reordering eigenvalues and eigenvectors
c permut : permuting eigenvalues and eigenvectors according to ind
c cg     : diagonalize a real matrix

      subroutine rdiag(nd,ndim,ar,ai,wr,wi,xr,xi,det,detr,ind)
c
c diagonalize an arbitrary real matrix
c
c nd   : dimensions
c ndim : rang of a
c ar   : matrix, real part
c ai             imaginary part = 0
c wr   : eigenvalues,  real part
c wi                   imaginary part
c xr   : eigenvectors, real part
c xi                   imaginary part
c det  : determinant
c detr : real part of the determinant
c ind  : 0 : no sorting eigenvalues
c        1      sorting eigenvalues
c      > 1      permuting columns according to ind
c
      implicit double precision(a-h,o-z)
      parameter(nd1=1000)
      dimension ar(nd,nd),ai(nd,nd),xr(nd,nd),xi(nd,nd),wr(nd),wi(nd)
      dimension fv1(nd1),fv2(nd1),fv3(nd1)
c     dimension intg(nd1)
 9000 format(' ifail =',i2)
      matz=1
      do i=1,ndim
        do j=1,ndim
          ai(i,j)=0.d0
...
c TEST WRITE 8
c      do ilinedex=1,nd
c         write(6666,6666)(ar(ilinedex,icoldex),icoldex=1,nd)
c      enddo
c      stop
c END TEST
c     call f02age(a,nd,ndim,wr,wi,xr,nd,xi,nd,intg,ifail)
      call cg(nd,ndim,ar,ai,wr,wi,matz,xr,xi,fv1,fv2,fv3,ifail)
c TEST WRITE 4
c      write(*,*)''
c      write(*,*)'   ~!!!~   TEST 4   ~!!!~   '
c      write(*,*)''
c END TEST 4
      if(ifail.ne.0) write(*,9000)ifail
      do j=1,ndim
        s=0.
        do i=1,ndim
          s=s+xr (i,j)**2+xi(i,j)**2
        enddo
        s=dsqrt(s)
        do i=1,ndim
          xr(i,j)=xr(i,j)/s
        ...
        enddo
      enddo
      if(ind.ne.0) call reore(nd,ndim,wr,wi,xr,xi)
      if(ind.gt.1) call permut(nd,ndim,wr,wi,xr,xi,ind)
      detr=1.d0
      det=1.d0
      do i=1,ndim
        detr=detr*wr(i)
        det=det*dsqrt(wr(i)**2+wi(i)**2)
      enddo
      if(detr.lt.0.d0)det=-det
c     det=detr
      return
      end

      subroutine reore(nd,ndim,wr,wi,xr,xi)
c
c reordering eigenvalues and eigenvectors
c
      implicit double precision(a-h,o-z)
      dimension wr(nd),wi(nd),xr(nd,nd),xi(nd,nd)
      ndim1=ndim-1
      do 30 i=1,ndim1
      i1=i+1
      do 20 j=i1,ndim
      awi=dsqrt(wr(i)**2+wi(i)**2)
      awj=dsqrt(wr(j)**2+wi(j)**2)
c     if(wr(j).gt.wr(i)) go to 20
      if(awj.gt.awi) go to 20
      w1=wr(i)
      w2=wi(i)
      wr(i)=wr(j)
      wi(i)=wi(j)
   ...
   30 continue
      ...
      subroutine permut(nd,ndim,wr,wi,xr,xi,ind)
c
c permuting eigenvalues and eigenvectors according to ind
c
      parameter(n1=4)
      implicit double precision(a-h,o-z)
      dimension wr(nd),wi(nd),xr(nd,nd),xi(nd,nd)
      dimension vr(n1),vi(n1),yr(n1,n1),yi(n1,n1),jnd(n1)
      call descomp(ndim,ind,jnd)
      do 10 i=1,ndim
      j=jnd(i)
      vr(i)=wr(j)
      vi(i)=wi(j)
   ...
   20 continue
      ...
      subroutine descomp(ndim,ind,jnd)
      dimension jnd(ndim)
c
c     jnd(1)=ind/1000
c     jnd(2)=ind/100-(ind/1000)*10
c     jnd(3)=ind/10 -(ind/100) *10
c     jnd(4)=ind    -(ind/10)  *10
c
      if(ndim.eq.1) then
        jnd(1)=ind
        return
      ...
      do i=2,ndim
        nn=1
        if(i.ne.ndim) nn=10**(ndim-i)
        jnd(i)=ind/nn-(ind/(nn*10))*10
      enddo
      write(*,1000)(jnd(i),i=1,ndim)
 1000 format(4i2)
      return
      end

      subroutine cpro(xr,xi,yr,yi,zr,zi)
c
c z=x*y
c
      implicit double precision(a-h,o-z)
      ...
      subroutine cdivi(xr,xi,yr,yi,zr,zi)
c
c z=x/y
c
      implicit double precision(a-h,o-z)
